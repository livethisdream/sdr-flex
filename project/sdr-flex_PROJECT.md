...
- **FM stereo.** `core.stereo` takes the composite and returns left and right — the
  38 kHz reference regenerated by doubling the pilot's *phase*, which is the only way to
  know it, and the two-by-two matrix after it. Two channels of `real`, because a channel
  count is a parameter of the type rather than a type of its own (ADR-0037), so nothing
  downstream had to learn anything: a slicer, an adapter and an export all still read the
...
- ~~**Stereo and RDS are visible, not decoded.**~~ Answered: RDS by the `redsea`
  adapter, stereo by `core.stereo`. What is left of it is smaller and worth naming: the
